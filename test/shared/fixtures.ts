...
export async function factoryFixture(_: Web3Provider, [wallet]: Wallet[]): Promise<FactoryFixture> {
  const factory = await deployContract(wallet, CoinSwapFactory, [wallet.address], overrides)
  return { factory }
}
...
export async function pairFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<PairFixture> {
  ...
  const pair = new Contract(pairAddress, JSON.stringify(CoinSwapPair.abi), provider).connect(wallet)
